...
class SpatialTransformer(nn.Module):
    ...
    接收一个 2D 图像特征图 (形状 B, C, H, W)。

    把它“压平”成一个 1D 序列 (形状 B, T, D)，就像把一幅画切成一个个小方块（token）并排成一行。

    使用内部的 BasicTransformerBlock（见下文）来处理这个序列。这一步就是 "Attention" 发生的地方。

    将处理后的 1D 序列“重塑”回 2D 图像特征图 (形状 B, C, H, W)。

    最后，把处理后的结果和原始输入相加（残差连接）。

    输入/输出？

    输入 1: hidden_states (形状 B, C, H, W) - 你的 2D 图像特征。

    输入 2: context (可选, 形状 B, Seq_len, D_ctx) - 外部信息，例如 (B, 77, 768) 形状的文本提示。

    输出: (B, C, H, W) - 一个“更懂上下文”的、形状相同的新 2D 图像特征。
