...
# [修复] 使用相对导入，因为 fgsa.py 就在当前目录下
try:
    from .fgsa import FGSA
except ImportError:
    # 备用方案
    import sys
    import os
...
# [修复] 导入组件注意力模块
try:
    from .Muti_attention import Attention as ComponentAttention
except ImportError:
    # 备用方案
    import sys
    import os
...
# 确保文件开头导入了 LazyConv2d (如果没有，可以使用 Conv2d 但需要知道输入维度，Lazy更方便)
try:
    from torch.nn import LazyConv2d
except ImportError:
    # 如果 PyTorch 版本较低不支持 LazyConv2d，需要手动计算，这里假设环境支持 (Torch > 1.8)
    pass

class StyleMemoryAttention(nn.Module):
    """
    PyTorch 实现的 Memory + Cross Attention 模块
    对应原 Paddle 代码中的 read_memory 和 read_decode 的核心逻辑
    """
    ...
    def forward(self, content_feat, style_feats):
        """
        Args:
            content_feat: (B, C, H, W) -> 作为 Query
            style_feats: (B, K_shot, C, H_s, W_s) 或 (B, C, H_s, W_s) -> 作为 Memory
        """
        B, C, H, W = content_feat.shape

        # 1. 准备 Memory (Key & Value)
        # 如果 style_feats 是 (B, C, H, W) 单张图，先升维
        if style_feats.dim() == 4:
            style_feats = style_feats.unsqueeze(1)  # (B, 1, C, H, W)

        B_s, K, C_s, Hs, Ws = style_feats.shape
        # 展平所有风格图的像素作为 Memory Bank
        memory_flat = style_feats.view(B, K * Hs * Ws, C)  # [B, S_len, C]

        # 生成 Key 和 Value
        k = self.to_k(memory_flat)
        v = self.to_v(memory_flat)

        # Reshape for Multi-head: [B, Num_Heads, S_len, D_k]
        k = k.view(B, -1, self.num_heads, self.d_k).transpose(1, 2)
        v = v.view(B, -1, self.num_heads, self.d_k).transpose(1, 2)

        # 2. 准备 Query
        q_flat = content_feat.flatten(2).transpose(1, 2)  # [B, H*W, C]
        residual = q_flat
        ...
        # 3. Cross Attention
        # Score = Q * K^T / sqrt(d) -> [B, Heads, T_len, S_len]
        ...
        # 还原回图像空间: [B, C, H, W]
        aligned_style = out.transpose(1, 2).view(B, C, H, W)

        return aligned_style


# -----------------------------------------------------------
# 新增 Block：MemoryAwareUpBlock2D (Coarse-to-Fine)
# -----------------------------------------------------------
class MemoryAwareUpBlock2D(nn.Module):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
            ...
            add_upsample=True,
            style_feature_dim=None, 
    ):
        super().__init__()
        resnets = []
        self.mid_convs = nn.ModuleList()
        self.component_attns = nn.ModuleList()
        self.fgsas = nn.ModuleList()
        ...
        for i in range(num_layers):
            ...
            # [修改] 在这里加入了 context_dim=256
            self.component_attns.append(
                ComponentAttention(
                    img_size=32, 
                    dim=res_skip_channels, 
            ...
            self.mid_convs.append(nn.Sequential(
                nn.Conv2d(res_skip_channels, res_skip_channels, 3, 1, 1, bias=False),
                nn.GroupNorm(32, res_skip_channels),
                nn.SiLU()
            ))

            self.fgsas.append(FGSA(res_skip_channels))
            
            # 使用 LazyConv2d 自动推断输入通道数
            self.style_adapters.append(nn.LazyConv2d(res_skip_channels, kernel_size=1))
    ...
    def forward(
            ...
            style_feats=None,
            component_feats_list=None,
            temb=None,
            upsample_size=None,
            **kwargs
    ):
        for i, (comp_attn, mid_conv, resnet, fgsa, style_adapter) in enumerate(
                zip(self.component_attns, self.mid_convs, self.resnets, self.fgsas, self.style_adapters)):

            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]

            # 1. Component Attention
            if component_feats_list is not None and len(component_feats_list) > 0:
                aligned_feat = comp_attn(res_hidden_states, component_feats_list)
            else:
                aligned_feat = res_hidden_states

            # 2. Smoothing
            refined = mid_conv(aligned_feat)

            # 3. FGSA (Style Aggregation)
            if style_feats is not None:
                curr_style = style_feats
                B, C_s, H_s, W_s = curr_style.shape
                _, _, H_c, W_c = refined.shape
                
                # 解包堆叠的 K-shot 特征
                if H_s > W_s * 2: 
                    try:
                        K_est = H_s // W_s 
                        curr_style = curr_style.view(B, C_s, K_est, W_s, W_s)
                        curr_style = curr_style.mean(dim=2) 
                    except:
                        pass 
                
                # 插值
                if curr_style.shape[-2:] != refined.shape[-2:]:
                    curr_style = F.interpolate(curr_style, size=refined.shape[-2:], mode='bilinear', align_corners=False)

                # 适配通道
                curr_style = style_adapter(curr_style)
                
                refined = fgsa(refined, curr_style)

            # 4. Concat & ResNet
            hidden_states = torch.cat([hidden_states, refined], dim=1)
...
def get_up_block(
    ...
    if up_block_type == "UpBlock2D":
        ...
        # === 新增注册 ===
    elif up_block_type == "MemoryAwareUpBlock2D":
        return MemoryAwareUpBlock2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            prev_output_channel=prev_output_channel,
...
class UNetMidMCABlock2D(nn.Module):
    ...
    def forward(
        ...
        for content_attn, style_attn, resnet in zip(self.content_attentions, self.style_attentions, self.resnets[1:]):
            ...
            current_style_feature = current_style_feature.permute(0, 2, 3, 1).reshape(batch_size, height * width,
                                                                                      channel)
...
class MCADownBlock2D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            print("The style_attention cross attention dim in Down Block {} layer is {}".format(i + 1,
                                                                                                cross_attention_dim))
            style_attentions.append(
                SpatialTransformer(
                    out_channels,
