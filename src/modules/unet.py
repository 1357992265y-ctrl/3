...
from .unet_blocks import (DownBlock2D,
                          UNetMidMCABlock2D,
                          UpBlock2D,
                          MemoryAwareUpBlock2D,  # <--- [新增] 必须添加这一行
                          get_down_block,
                          get_up_block)
...
class UNet(ModelMixin, ConfigMixin):
    ...
    def forward(
            ...
            content_encoder_downsample_size: int = 4,
            component_list: list = None,  # 接收编码后的特征列表
        ...
        style_feature_list = None
        if isinstance(encoder_hidden_states, (tuple, list)):
            # 假设最后一个元素是特征列表
            style_feature_list = encoder_hidden_states[-1]
            # 5. up
            style_feature_list = None
            if isinstance(encoder_hidden_states, (tuple, list)):
                style_feature_list = encoder_hidden_states[-1]
            ...
            for i, upsample_block in enumerate(self.up_blocks):
                ...
                # [关键修改] 识别 MemoryAwareUpBlock2D 并传入 component_list
                if isinstance(upsample_block, MemoryAwareUpBlock2D):
                    # 确定 Style Features (来自 Global Style Encoder)
                    current_style_feat = None
                    if style_feature_list is not None:
                        # 这里的索引逻辑需要根据您的架构层数调整，这里仅做示例
                        idx = -(i + 1)
                        if abs(idx) <= len(style_feature_list):
                            current_style_feat = style_feature_list[idx]

                    sample = upsample_block(
                        hidden_states=sample,
                        res_hidden_states_tuple=res_samples,
                        style_feats=current_style_feat,  # 全局风格
                        component_feats_list=component_list,  # [新增] 组件特征列表
                        temb=emb,
