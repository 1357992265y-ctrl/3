...
def upfirdn2d_native(tensor, kernel, up=1, down=1, pad=(0, 0)):
    """
这部分函数不包含可学习的权重（Weights），它们是纯数学运算，用于高质量地调整图像大小。

upfirdn2d_native:

作用: 这是最底层的“引擎”。它在一个函数里同时完成 上采样 (Up) -> 滤波 (Fir) -> 下采样 (Dn)。

目的: 防止图像缩放时出现锯齿（抗锯齿）。

输入: 张量、滤波核（Kernel）、放大倍数、缩小倍数。
...
输出: 调整大小后的张量。
    ...
    # c针对mps特定问题的临时解决方法：
    #     # https://github.com/pytorch/pytorch/issues/84535
...
def upsample_2d(hidden_states, kernel=None, factor=2, gain=1):
    r"""
    作用: upfirdn2d_native 的包装器，专门用于放大图像。
特点: 它会自动创建一个平滑滤波器（通常是 [1, 1] 这种），确保放大后的图像不会过于像素化。
...
def downsample_2d(hidden_states, kernel=None, factor=2, gain=1):
    r"""
    作用: upfirdn2d_native 的包装器，专门用于缩小图像。
特点: 类似于平均池化，但数学上更严谨，防止高频信息混叠。
...
class Mish(torch.nn.Module):
    """
    作用: 一个激活函数（类似 ReLU，但更平滑）。

输入: 任意数据。

输出: 经过非线性变换的数据（保留负值的一点梯度）。
    """
...
Conv (卷积): 使用步长为 2 的卷积来缩小图像（可学习，能变聪明）。

AvgPool (平均池化): 简单的数学平均（不可学习，固定规则）。
...
ConvTranspose (转置卷积): 一步到位的可学习放大。

Interpolate + Conv: 先强制拉大图片（插值），再用卷积修补细节。
