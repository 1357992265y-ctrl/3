...
class StyleEncoder(ModelMixin, ConfigMixin):
    ...
    Supports K-shot input: [Batch, K, 3, H, W]
    """
    ...
    def forward(self, x):
        # [修改] 支持 K-shot 输入: [Batch, K, 3, H, W]
        # 如果是 5D 张量，说明输入了多张风格图
        is_k_shot = False
        if x.dim() == 5:
            is_k_shot = True
            B, K, C, H, W = x.shape
            # 合并 Batch 和 K 维度: [B*K, C, H, W]
            x = x.view(B * K, C, H, W)
        else:
            B = x.shape[0]
            K = 1
        ...
        # 逐层提取特征
        for index, blocklist in enumerate(self.blocks[:-1]):  # Exclude last layer first
            for block in blocklist:
                h = block(h)
            if index in self.save_featrues[:-1]:
                residual_features.append(h)

        h = self.blocks[-1](h)  # Last layer
        style_emd = h  # Spatial Features: [B*K, C_out, H_out, W_out]

        # Global Vector
        vec = F.adaptive_avg_pool2d(h, (1, 1))
        vec = vec.view(vec.size(0), -1)  # [B*K, C_out]

        # [修改] 恢复维度并聚合
        if is_k_shot:
            # 1. 对 residual_features 进行处理
            # 这里的策略是：将 K 张图的特征在空间维度上堆叠（相当于拼接成更长的序列），供 Cross-Attention 使用
            new_residuals = []
            for res in residual_features:
                _, C_r, H_r, W_r = res.shape
                # Reshape: [B, K, C, H, W] -> [B, C, K*H, W]
                # 这样对于 UNet 来说，就像是输入了一张高度为 K*H 的长图，Attn 会自动将其展平处理
                res_reshaped = res.view(B, K, C_r, H_r, W_r)
                res_stacked = res_reshaped.permute(0, 2, 1, 3, 4).reshape(B, C_r, K * H_r, W_r)
                new_residuals.append(res_stacked)
            residual_features = new_residuals

            # 2. 对 style_emd (Spatial Feature) 进行处理
            # 同样堆叠: [B, C, K*H, W]
            _, C_s, H_s, W_s = style_emd.shape
            style_emd = style_emd.view(B, K, C_s, H_s, W_s).permute(0, 2, 1, 3, 4).reshape(B, C_s, K * H_s, W_s)

            # 3. 对 vec (Global Vector) 进行处理
            # 全局向量通常用于 AdaIN 或 TimeEmbedding Modulation，需要每个 Batch 一个向量
            # 所以这里我们取 K 个向量的【平均值】
            vec = vec.view(B, K, -1).mean(dim=1)  # [B, C_out]

        return style_emd, vec, residual_features