...
class FontDiffuserDPMPipeline():
    """cFontDiffuser管道搭配DPM_Solver调度器。
    """
    ...
    def generate(
        ...
        # === 关键修改：将 component_list 放入 kwargs 传递给 model_wrapper ===
        # 这样 model_wrapper 内部调用 model() 时就会带上它
        if component_list is not None:
            doubled_component_list = []
            for comp in component_list:
                # 假设 comp 是 (B, C, H, W)，复制成 (2B, C, H, W)
                doubled_component_list.append(torch.cat([comp, comp], dim=0))
            model_kwargs["component_list"] = doubled_component_list
        # ===============================================================
        cond = []
