...
def image_process(args, content_image=None, style_image=None):
    ...
    component_list = []
    if args.character_input:
        char = args.content_character
        # 伪代码：获取该字符的部件列表
        # components = get_components_from_char(char)
        # for comp_img in components:
        #     component_list.append(transform(comp_img))
    # ========================

    return content_image, style_image, content_image_pil, component_list # <--- 返回组件
...
def sampling(args, pipe, content_image=None, style_image=None):
    ...
    content_image, style_image, content_image_pil, component_list = image_process(...)
    ...
    with torch.no_grad():
        ...
        images = pipe.generate(
            ...
            method=args.method,
            component_list=component_list,  # <--- 传入 Pipeline
